...
- Suppose you have more than 1000 products in your database. How would you implement a method for supporting pagination and return pages of 50 products to your frontend?
	- Creating a method in the repository interface and making use of **Page** and **Pageable** and requesting it with **PageRequest**
...
- How many products are returned by the *findByDescriptionContaining* query? Why?
	- 2 products, because **PageRequest**is setting the number of elements per page to 2, and currently is showing only the first page.
- Which are the collection names where the objects are stored? Where are those names assigned?
	- customer and product,When you extend the repository(CrudRepository or MongoRepository), you define the object of the model to be stored and the name of the collection will be the same as the model.
